...
class DeliveryAddress(models.Model):
    ...
    contact_name = fields.Char(
        string="Nombre Contacto", help="Persona responsable de recibir la entrega"
    )

    contact_phone = fields.Char(
        string="Teléfono Contacto", help="Teléfono del responsable de recepción"
    )
    ...
    full_address = fields.Char(
        string="Dirección Completa", compute="_compute_full_address", store=False
    )
            ...
            if address.is_default:
                ...
                if other_defaults:
                    raise ValidationError(
                        _(
                            "Ya existe una dirección predeterminada para este distribuidor. "
    ...
    def create(self, vals):
        ...
        if "partner_id" in vals and not vals.get("is_default"):
            existing_addresses = self.search(
                [("partner_id", "=", vals["partner_id"]), ("active", "=", True)],
                limit=1,
            )
            ...
            if not existing_addresses:
                ...
                _logger.info(
                    f"Primera dirección para partner {vals['partner_id']}, marcada como predeterminada"
                )
        ...
        if vals.get("is_default"):
            for address in self:
                ...
                if other_defaults:
                    ...
                    _logger.info(
                        f"Desmarcadas {len(other_defaults)} direcciones predeterminadas para partner {address.partner_id.name}"
                    )
    ...
    def action_set_default(self) -> dict:
        ...
        _logger.info(
            f"Dirección {self.name} marcada como predeterminada para {self.partner_id.name}"
        )
        ...
        return {
            ...
            "params": {
                ...
                "message": _('La dirección "%s" se ha marcado como predeterminada.')
                % self.name,
                "type": "success",
                "sticky": False,
            },
    ...
    def action_archive(self) -> dict:
        ...
        if self.is_default:
            ...
            if other_address:
                ...
                _logger.info(
                    f"Dirección {other_address.name} marcada como predeterminada al archivar {self.name}"
                )
