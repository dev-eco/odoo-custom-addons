...
class SaleOrder(models.Model):
    ...
    @api.constrains('early_payment_due_date', 'commitment_date')
    def _check_early_payment_date(self):
        for order in self:
            if (order.early_payment_due_date and order.commitment_date and 
                order.early_payment_due_date > order.commitment_date):
                raise ValidationError(
                    'La fecha límite para descuento no puede ser posterior al vencimiento del pago.'
                )
