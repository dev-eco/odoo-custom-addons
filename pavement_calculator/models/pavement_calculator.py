...
class PavementCalculator(models.Model):
    _name = 'pavement.calculator'
    _description = 'Calculadora de Pavimento de Caucho'
    _rec_name = 'name'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    
    name = fields.Char(
        'Referencia', 
        default=lambda self: _('Nueva Calculadora'),
        readonly=True, 
        copy=False
    )
    ...
    waste_factor = fields.Float(
        ...
        required=True,
        default=5.0,
        help="Porcentaje adicional para compensar pérdidas",
        tracking=True,
    )
    ...
    sale_order_id = fields.Many2one(
        ...
        readonly=True,
        copy=False,
        help="Presupuesto generado o al que se añadirán las líneas"
    )
    ...
    state = fields.Selection([
        ...
        ('sale_order', 'Presupuesto Generado'),
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', _('Nueva Calculadora')) == _('Nueva Calculadora'):
                vals['name'] = self.env['ir.sequence'].next_by_code('pavement.calculator') or _('Nueva Calculadora')
    ...
    def action_confirm(self):
        """Confirma el cálculo y bloquea la edición de los campos principales"""
        for record in self:
            record.state = 'confirmed'
    
    def action_draft(self):
        """Vuelve al estado de borrador para permitir edición"""
        for record in self:
            if record.state != 'sale_order':
                record.state = 'draft'
    
    def action_create_sale_order(self):
        """Crear un presupuesto basado en los cálculos"""
        self.ensure_one()
        ...
        # Actualizar el registro de la calculadora
        self.write({
        ...
        message = _("Se ha creado el presupuesto %s basado en esta calculadora.") % sale_order.name
        self.message_post(body=message)
        
        # Devolver acción para ver el presupuesto
        return {
