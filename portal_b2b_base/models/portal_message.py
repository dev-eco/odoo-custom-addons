...
class PortalMessage(models.Model):
    ...
    body = fields.Html(string="Cuerpo HTML", help="Versión HTML del mensaje")
    ...
    user_id = fields.Many2one(
        "res.users", string="Responsable", help="Usuario asignado para responder"
    )

    is_read = fields.Boolean(string="Leído", default=False, index=True, tracking=True)
    ...
    related_id = fields.Integer(
        string="ID Relacionado", help="ID del registro relacionado"
    )
    ...
    child_ids = fields.One2many("portal.message", "parent_id", string="Respuestas")

    reply_count = fields.Integer(
        string="Número de Respuestas", compute="_compute_reply_count", store=True
    )
