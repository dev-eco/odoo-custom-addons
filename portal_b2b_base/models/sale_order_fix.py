...
class SaleOrder(models.Model):
    ...
    def action_fix_portal_visibility_all(self):
        ...
        _logger.info(f"Iniciando recálculo de portal_visible para {len(orders)} pedidos...")
        ...
        for order in orders:
            ...
            if order.portal_visible != old_value:
                count_fixed += 1
                _logger.debug(f"Pedido {order.name}: portal_visible cambiado de {old_value} a {order.portal_visible}")
    ...
    def action_fix_portal_visibility_partner(self):
        ...
        _logger.info(f"Recalculando portal_visible para {len(orders)} pedidos del partner {partner.name}...")
        ...
        return {
            ...
            'params': {
                'title': _('Recálculo Completado'),
                'message': _('%s pedidos fueron actualizados para %s.') % (count_fixed, partner.name),
                'type': 'success',
            }
        }
