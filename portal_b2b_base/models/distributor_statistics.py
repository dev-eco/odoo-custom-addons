...
class DistributorStatistics(models.Model):
    ...
    period_end = fields.Date(
        string="Fin Período", required=True, default=fields.Date.today
    )

    # Métricas de pedidos (sin store para evitar recalculos costosos)
    total_orders = fields.Integer(
        string="Total Pedidos", compute="_compute_order_metrics", store=False
    )

    confirmed_orders = fields.Integer(
        string="Pedidos Confirmados", compute="_compute_order_metrics", store=False
    )

    cancelled_orders = fields.Integer(
        string="Pedidos Cancelados", compute="_compute_order_metrics", store=False
    )
    ...
    total_products_ordered = fields.Integer(
        string="Productos Pedidos", compute="_compute_product_metrics", store=False
    )
    ...
    def _compute_order_metrics(self):
        ...
        for stat in self:
            ...
            stat.confirmed_orders = len(
                orders.filtered(lambda o: o.state in ["sale", "done"])
            )
            ...
            stat.average_order_value = (
                stat.total_amount / stat.total_orders if stat.total_orders > 0 else 0.0
            )
    ...
    def _compute_product_metrics(self):
        ...
        for stat in self:
            ...
            stat.total_products_ordered = int(
            ...
            for line in order_lines:
                if line.product_id:
                    product_qty[line.product_id.id] = (
                        product_qty.get(line.product_id.id, 0) + line.product_uom_qty
                    )

            top_products = sorted(
                product_qty.items(), key=lambda x: x[1], reverse=True
            )[:5]
    ...
    def _compute_invoice_metrics(self):
        ...
        for stat in self:
            ...
            stat.total_paid = sum(
                invoices.filtered(lambda i: i.payment_state == "paid").mapped(
                    "amount_total"
                )
            )
    ...
    def get_chart_data_top_products(self, limit=10):
        ...
        sorted_products = sorted(
            product_data.values(), key=lambda x: x["qty"], reverse=True
        )[:limit]
    ...
    def get_kpi_summary(self):
        ...
        if previous_amount > 0:
            amount_variation = (
                (self.total_amount - previous_amount) / previous_amount
            ) * 100

        count_variation = 0
        if previous_count > 0:
            count_variation = (
                (self.confirmed_orders - previous_count) / previous_count
            ) * 100
