...
class SaleReturn(models.Model):
    ...
    name = fields.Char(
        string="Número RMA", required=True, copy=False, readonly=True, default="/"
    )
    ...
    return_date = fields.Date(
        string="Fecha Devolución", default=fields.Date.today, required=True
    )
    ...
    line_ids = fields.One2many(
        "sale.return.line", "return_id", string="Líneas de Devolución", copy=True
    )
    ...
    customer_notes = fields.Text(
        string="Notas del Distribuidor", help="Notas adicionales del distribuidor"
    )
    ...
    rejection_reason = fields.Text(string="Motivo Rechazo", readonly=True)
    ...
    def create(self, vals):
        ...
        if not self.env.user.has_group("base.group_user"):
            ...
            if vals.get("name", "/") == "/":
                vals["name"] = (
                    self.env["ir.sequence"].next_by_code("sale.return") or "/"
                )
    ...
    def action_submit(self):
        ...
        return {
            ...
            "params": {
                ...
                "message": _(
                    "Su solicitud de devolución ha sido enviada para revisión."
                ),
                "type": "success",
                "sticky": False,
            },
    ...
    def _create_credit_note(self):
        ...
        _logger.info(
            f"Nota de crédito {credit_note.name} creada para devolución {self.name}"
        )
...
class SaleReturnLine(models.Model):
    ...
    return_id = fields.Many2one(
        "sale.return", string="Devolución", required=True, ondelete="cascade"
    )

    product_id = fields.Many2one(
        "product.product", string="Producto", required=True, ondelete="restrict"
    )

    quantity = fields.Float(string="Cantidad", default=1.0, required=True)

    unit_price = fields.Monetary(
        string="Precio Unitario", currency_field="currency_id", required=True
    )
    ...
    currency_id = fields.Many2one(
        "res.currency", related="return_id.currency_id", store=True
    )
