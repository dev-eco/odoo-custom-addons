...
class AccountMove(models.Model):
    ...
    def _ensure_access_tokens(self):
        ...
        if invoices_without_token:
            _logger.info(
                f"Generando access_token para {len(invoices_without_token)} facturas..."
            )
            for invoice in invoices_without_token:
                invoice.access_token = self._generate_access_token()
            _logger.info(
                f"✓ Tokens generados para {len(invoices_without_token)} facturas"
            )
...
class SaleOrder(models.Model):
    ...
    customer_delivery_note_filename = fields.Char(string="Nombre Archivo Albarán")
    ...
    def _compute_portal_visible(self) -> None:
        ...
        for order in self:
            order.portal_visible = (
                bool(order.partner_id.user_ids) and order.state != "cancel"
            )
    ...
    @api.depends(
        "distributor_delivery_note_ids",
        "distributor_invoice_ids",
        "distributor_label_ids",
        "distributor_transport_invoice_ids",
    ...
    @api.depends(
        "state", "distributor_document_count", "distributor_documents_reviewed"
    )
    def _compute_has_new_documents(self) -> None:
        ...
        for order in self:
            order.has_new_distributor_documents = (
                ...
                and order.distributor_document_count > 0
                and not order.distributor_documents_reviewed
            )
    ...
    def _ensure_access_tokens(self):
        ...
        if orders_without_token:
            _logger.info(
                f"Generando access_token para {len(orders_without_token)} pedidos..."
            )
    ...
    def _ensure_order_status_defaults(self):
        ...
        orders_without_status = self.search([("order_status", "=", False)])
        if orders_without_status:
            orders_without_status.write({"order_status": "new"})
            _logger.info(
                f"Actualizados {len(orders_without_status)} pedidos sin order_status"
            )
    ...
    def validar_credito_antes_confirmar(self) -> bool:
        ...
        if partner.credit_limit <= 0:
            ...
            _logger.info(
                f"Pedido {self.name}: distribuidor sin límite de crédito configurado"
            )
            return True
    ...
    def action_cancel_from_portal(self) -> dict:
        ...
        _logger.info(
            f"Pedido {self.name} cancelado desde portal por usuario {self.env.user.name}"
        )
        ...
        return {
            ...
            "params": {
                ...
                "message": _("El pedido %s ha sido cancelado correctamente.")
                % self.name,
                "type": "success",
                "sticky": False,
            },
    ...
    def _ensure_computed_fields(self):
        ...
        try:
            ...
            _logger.info(
                f"Campos computed actualizados en {len(orders_to_update)} pedidos"
            )
