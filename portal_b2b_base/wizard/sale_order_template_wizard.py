...
class SaleOrderTemplateWizard(models.TransientModel):
    ...
    def action_create_template(self):
        ...
        if not order.order_line:
            raise ValidationError(
                _('El pedido no tiene líneas de productos.')
            )
        ...
        template = self.env['sale.order.template'].create({
            ...
            'delivery_address_id': order.delivery_address_id.id if self.include_delivery_address and hasattr(order, 'delivery_address_id') else None,
            'distributor_label_id': order.distributor_label_id.id if self.include_distributor_label and hasattr(order, 'distributor_label_id') else None,
        })
