...
class DistributorAPI(http.Controller):
    ...
    @http.route(
        "/api/distributor/credit_status", type="json", auth="user", website=True
    )
    def get_credit_status(self):
        ...
        try:
            ...
            if not hasattr(partner, "is_distributor") or not partner.is_distributor:
                return {"success": False, "error": _("Usuario no autorizado")}
    ...
    def mark_notification_read(self, notification_id):
        ...
        try:
            ...
            if not notification.exists():
                return {"success": False, "error": _("Notificación no encontrada")}
            ...
            _logger.error(
                f"Error marking notification as read: {str(e)}", exc_info=True
            )
            return {"success": False, "error": str(e)}
    ...
    @http.route(
        "/api/productos/<int:product_id>/info", type="json", auth="user", website=True
    )
    def get_product_info(self, product_id):
        ...
        try:
            ...
            pricelist = (
                partner.obtener_tarifa_aplicable()
            ...
            if pricelist:
                price = pricelist._get_product_price(
                    product.product_variant_id, 1.0, partner=partner
                )
            else:
