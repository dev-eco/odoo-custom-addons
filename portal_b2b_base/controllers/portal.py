...
class PortalB2B(CustomerPortal):
    ...
    def portal_order_page(
        ...
        **kw,
            ...
            try:
                ...
                if access_token:
                    ...
                    if not request.env.user._is_public():
                        ...
                        if (
                            ...
                            _logger.warning(
                                f"Usuario {request.env.user.login} intentó acceder a pedido {order_id} sin permiso"
                            )
                            return request.redirect("/mis-pedidos")
                    else:
                        # Usuario público sin token
                        _logger.warning(
                            f"Intento de acceso público a pedido {order_id} sin token"
                        )
                ...
                if report_type == "pdf":
                    ...
                    pdfhttpheaders = [
                        ...
                        (
                            "Content-Disposition",
                            f'attachment; filename="{order_sudo.name}.pdf"',
                        ),
                    ]
        ...
        if access_token:
            try:
                return super().portal_order_page(
                    order_id=order_id, access_token=access_token, message=message, **kw
                )
            except Exception as e:
            ...
            if partner.is_distributor:
                ...
                if report_type in ("html", "pdf", "text"):
                    ...
                    return request.redirect(
                        f"/mis-facturas/{invoice_id}?error=download_blocked"
                    )
            ...
            try:
                ...
                if access_token:
                    ...
                    if not request.env.user._is_public():
                        partner = request.env.user.partner_id
                        if (
                            invoice_sudo.partner_id.commercial_partner_id
                            != partner.commercial_partner_id
                        ):
                            _logger.warning(
                                f"Usuario {request.env.user.login} intentó acceder a factura {invoice_id} sin permiso"
                            )
                            return request.redirect("/mis-facturas")
                    else:
                        # Usuario público sin token
                        _logger.warning(
                            f"Intento de acceso público a factura {invoice_id} sin token"
                        )
                        return request.redirect("/web/login")
                ...
                if report_type == "pdf":
                    ...
                    pdfhttpheaders = [
                        ...
                        (
                            "Content-Disposition",
                            f'attachment; filename="{invoice_sudo.name}.pdf"',
                        ),
                    ]
                ...
                _logger.error(
                    f"Error en descarga de factura {invoice_id}: {str(e)}",
                    exc_info=True,
                )
                return request.redirect("/mis-facturas")
    ...
    def _get_orders_domain(self):
        ...
        _logger.debug(
            f"Portal orders filter - User: {request.env.user.name}, "
    ...
    def _get_invoices_domain(self, invoice_type=None):
        ...
        _logger.debug(
            f"Portal invoices filter - Related partners: {len(related_partners)}"
        )
    ...
    @http.route(
        ["/mis-pedidos", "/mis-pedidos/page/<int:page>"],
        type="http",
        auth="user",
        website=True,
    )
    def portal_mis_pedidos(
        ...
        **kw,
        ...
        searchbar_inputs = {
            ...
            "client_order_ref": {
                "input": "client_order_ref",
                "label": "Buscar en Su Referencia",
            },
        ...
        try:
            ...
            _logger.error(
                f"Error renderizando template portal_mis_pedidos: {str(e)}",
                exc_info=True,
            )
    ...
    @http.route(
        ["/mis-pedidos/<int:order_id>/cancelar"],
        type="http",
        auth="user",
        website=True,
        methods=["POST"],
    ...
    @http.route(
        ["/mis-pedidos/<int:order_id>/repetir"], type="http", auth="user", website=True
    )
    ...
    @http.route(
        ["/crear-pedido/submit"], type="json", auth="user", methods=["POST"], csrf=True
    )
    def portal_crear_pedido_submit(self, **kw):
        ...
        try:
            ...
            for line_data in lines_data:
                try:
                    ...
                    if product_id and qty > 0:
                        valid_lines.append({"product_id": int(product_id), "qty": qty})
            ...
            if not valid_lines:
                return {
                    "error": _("Debe agregar al menos un producto con cantidad válida")
                }
            ...
            if delivery_option == "saved":
                ...
                if delivery_address_id:
                    try:
                        ...
                        if (
                            ...
                            and delivery_address.partner_id == partner
                        ):
                ...
                try:
                    new_address_vals = {
                        ...
                        "require_appointment": kw.get("new_address_appointment")
                        == "on",
                    ...
                    if not all(
                        [
                            new_address_vals["name"],
                            new_address_vals["street"],
                            new_address_vals["zip"],
                            new_address_vals["city"],
                        ]
                    ):
                        return {
                            "error": _(
            ...
            for line_data in valid_lines:
                try:
                    product = (
                        ...
                        .sudo()
                        .browse(line_data["product_id"])
                    )

                    if not product.exists():
                        _logger.warning(
                            f"Producto {line_data['product_id']} no encontrado"
                        )
                        continue
            ...
            _logger.error(
                f"Error al crear pedido desde portal: {str(e)}", exc_info=True
            )
            return {
                "error": _("Error al crear el pedido. Por favor, inténtelo de nuevo.")
            }
    ...
    def portal_exportar_pedidos(self, date_begin=None, date_end=None, **kw):
        ...
        try:
            ...
            for order in orders:
                ...
                # Estados en español
                worksheet.write(
                    row, 3, state_translations.get(order.state, order.state)
                )
                worksheet.write(
                    row,
                    4,
                    order_status_translations.get(
                        order.order_status, order.order_status or ""
                    ),
                )
                worksheet.write(
                    row,
                    5,
                    picking_status_translations.get(
                        order.picking_status, order.picking_status or ""
                    ),
                )
                ...
                if (
                    ...
                    and order.distributor_customer_name
                ):
            ...
            summary_sheet.write(
                2,
                0,
                f'Período: {date_begin or "Desde inicio"} - {date_end or "Hasta hoy"}',
            )
    ...
    @http.route(
        ["/mis-facturas/<int:invoice_id>"], type="http", auth="user", website=True
    )
    def portal_factura_detalle(
        self, invoice_id, report_type=None, access_token=None, **kw
    ):
        ...
        if report_type == "pdf":
            ...
            try:
                ...
                if not invoice_sudo.access_token:
                    invoice_sudo.sudo().access_token = (
                        invoice_sudo._generate_access_token()
                    )
                    _logger.info(f"Token generado para factura {invoice_sudo.name}")
                ...
                _logger.info(
                    f"PDF generado correctamente para factura {invoice_sudo.name}"
                )

                return request.make_response(pdf_content, headers=pdfhttpheaders)
            except Exception as e:
                _logger.error(
                    f"Error generando PDF de factura {invoice_id}: {str(e)}",
                    exc_info=True,
                )
    ...
    @http.route(
        ["/mis-facturas", "/mis-facturas/page/<int:page>"],
        type="http",
        auth="user",
        website=True,
    )
    def portal_mis_facturas(
        self, page=1, date_begin=None, date_end=None, sortby=None, search=None, **kw
    ):
    ...
    @http.route(
        ["/mi-cuenta/actualizar"], type="json", auth="user", methods=["POST"], csrf=True
    )
    ...
    def portal_create_return_submit(self, **kw):
        ...
        try:
            ...
            _logger.info(
                f"Devolución creada y enviada por {partner.name} para pedido {order.name}"
            )
    ...
    def portal_templates(self, **kw):
        ...
        try:
            templates = request.env["sale.order.template"].search(
                [("partner_id", "=", partner.id)], order="name asc"
            )
        except Exception as e:
    ...
    @http.route(
        ["/mis-plantillas/<int:template_id>"], type="http", auth="user", website=True
    )
    ...
    def portal_statistics_dashboard(self, period="month", **kwargs):
        ...
        try:
            ...
            top_products = sorted(
                product_stats.values(), key=lambda x: x["quantity"], reverse=True
            )[:10]
    ...
    def portal_activity_history(self, page=1, action_filter=None, **kw):
        ...
        try:
            ...
            logs = request.env["portal.audit.log"].search(
                domain, limit=50, offset=pager["offset"], order="create_date desc"
            )
        except Exception as e:
    ...
    @http.route(
        ["/api/notificaciones/marcar-leida"], type="json", auth="user", methods=["POST"]
    )
    ...
    @http.route(
        ["/mis-pedidos/<int:order_id>/documentos"],
        type="http",
        auth="user",
        website=True,
    )
    ...
    @http.route(
        ["/mis-pedidos/<int:order_id>/subir-documento"],
        type="http",
        auth="user",
        website=True,
        methods=["POST"],
    ...
    def portal_upload_document(self, order_id, document_type, **post):
        ...
        try:
            ...
            order.sudo().write({field_name: [(4, attachment.id)]})
    ...
    @http.route(
        ["/mis-pedidos/<int:order_id>/eliminar-documento/<int:attachment_id>"],
        type="http",
        auth="user",
        website=True,
        methods=["POST"],
    ...
    @http.route(
        ["/api/preferencias/actualizar"], type="json", auth="user", methods=["POST"]
    )
    def api_update_preferences(self, **preferences):
        ...
        try:
            ...
            _logger.info(
                f"Preferencias actualizadas para {partner.name}: {preferences}"
            )
    ...
    @http.route(
        ["/api/preferencias/obtener"], type="json", auth="user", methods=["POST"]
    )
    ...
    @http.route(
        ["/api/productos/<int:product_id>/stock"],
        type="json",
        auth="user",
        methods=["POST"],
    )
    def api_producto_stock(self, product_id, **kw):
        ...
        try:
            ...
            return {"message": "Información de stock no disponible para distribuidores"}
    ...
    @http.route(
        ["/api/productos/<int:product_id>/historial-precios"],
        type="json",
        auth="user",
        methods=["POST"],
    )
    ...
    def api_productos_catalogo(
        self, page=1, limit=20, search="", category_id=None, sort="name", **kw
    ):
    ...
    @http.route(
        ["/api/productos/categorias"], type="json", auth="user", methods=["POST"]
    )
        ...
        try:
            ...
            if partner.allow_all_categories or not partner.allowed_product_categories:
                ...
                categories = (
                    ...
                    .sudo()
                    .search([("product_count", ">", 0)], order="name asc")
                )
            else:
                ...
                categories = (
                    request.env["product.category"].sudo().browse(allowed_category_ids)
                )
    ...
    @http.route(
        ["/api/notificaciones/recientes"], type="json", auth="user", methods=["POST"]
    )
    def get_recent_notifications(self, limit=10, **kw):
        ...
        try:
            notifications = (
                request.env["portal.notification"]
                .sudo()
                .get_recent_notifications(partner.id, limit=limit)
            )

            unread_count = (
                request.env["portal.notification"].sudo().get_unread_count(partner.id)
            )
    ...
    @http.route(
        ["/api/notificaciones/<int:notification_id>/marcar-leida"],
        type="json",
        auth="user",
        methods=["POST"],
    )
    def mark_notification_read(self, notification_id, **kw):
        ...
        try:
            notification = (
                request.env["portal.notification"].sudo().browse(notification_id)
            )
