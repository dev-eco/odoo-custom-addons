...
class PortalStatistics(http.Controller):
    ...
    def portal_statistics(self, period='month', **kw):
        ...
        stats = request.env['distributor.statistics'].sudo().search([
        ...
        if not stats:
            stats = request.env['distributor.statistics'].sudo().create({
                'partner_id': partner.id,
                'period_start': period_start,
                'period_end': period_end,
            })
    ...
    @http.route(['/api/estadisticas/grafico'], type='json', auth='user', methods=['POST'])
    ...
    def _get_top_products(self, partner_id, start_date, end_date):
        ...
        for line in order_lines:
            if line.product_id:
                product_qty[line.product_id.name] = product_qty.get(line.product_id.name, 0) + line.product_uom_qty
